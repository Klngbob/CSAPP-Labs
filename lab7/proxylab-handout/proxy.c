...
void set_requesthdrs(RequestLine *request_line, RequestHeader *request_headers, int *hdrs_nums)
{
    ...
    {
        if (!strcasecmp(request_headers[i].key, "Host"))
            host_flag = i;
        else if (!strcasecmp(request_headers[i].key, "User-Agent"))
            usr_flag = i;
        else if (!strcasecmp(request_headers[i].key, "Connection"))
            conn_flag = i;
        else if (!strcasecmp(request_headers[i].key, "Proxy-Connection"))
            proxy_flag = i;
    }
    if (host_flag == -1)
    {
    ...
    else
    {
        for (i = 0; i < MAX_CACHE_SIZE; ++i)
        {
            ...
            if (cache.cache_line[i].readcnt == 1)
                P(&cache.cache_line[i].w);
            V(&cache.cache_line[i].mutex);
            if (cache.cache_line[i].ts > max_ts)
            { /* 替换ts最大的cache块 */
                res = i;
            }
            P(&cache.cache_line[i].mutex);
